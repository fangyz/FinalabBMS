...
function getPageData(pageIndex) {
    //var TypeId = $("#questionTypeId").value;
    ...
    $.getJSON("/JoinUs/QuestionManage/GetPageDatatype?typeId=" + TypeId + "&content=" + Content +"&pageIndex=" + pageIndex + "&pageSize=10" ,function (jsonObj) {
        if (jsonObj.Statu == "ok") {
            //隐藏域存储信息
            $("#page_index_now").val(pageIndex);
            $("#page_count").val(jsonObj.PageCount);
            $("#question_index_now").val(TypeId);
            $("#question_content_now").val(Content);
            //取出列表数据
            list = jsonObj.Data;
            //json数据放入表格
            jsonIntoTb(list);
            ...
            //url同步改变
            history.replaceState(null, null, "/JoinUs/QuestionManage/Indextype/" + pageIndex);
        }
    });
}
