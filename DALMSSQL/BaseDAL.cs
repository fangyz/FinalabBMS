...
namespace DALMSSQL
{
    public class BaseDAL<T> : IDAL.IBaseDAL<T> where T : class,new()
    {
        ...
        /// <returns></returns>
        public List<T> GetPagedList<TKey>(int pageIndex, int pageSize, ref int rowCount, Expression<Func<T, bool>> whereLambda, Expression<Func<T, TKey>> orderBy, bool isAsc)
        {
        ...
        #endregion
        #region 1.1新增实体，并返回实体（目的获取插入数据后的自增的Id）+public T AddGetAutoId(T model)
        /// <summary>
        /// 新增实体，并返回实体（目的获取插入数据后的自增的Id）
        /// </summary>
